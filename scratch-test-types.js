...
async function testFetch(ref, type) {
  try {
    ...
    console.log(`Type ${type}: Response Status: ${res.status} ${res.statusText}`);
    const text = await res.text();
    console.log(`Type ${type}: Response Body:`, text);
  } catch (err) {
    console.error(`Type ${type} failed:`, err.message);
  }
}

async function run() {
  await testFetch('MRSU4690575', 'equipment');
  await testFetch('MRSU4690575', 'booking');
  await testFetch('MRSU4690575', 'transport_document');
}
